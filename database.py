    ...
    #Создание таблицы
        cursor.execute("""CREATE TABLE users_db
                    (id integer, name text, last_name text,
                     status text, days_left, time, mode) """)
...
def update_status(chat_id, name, last_name, status, days, time, mode):
    check_init()
    conn=sqlite3.connect(file_name)
    curs = conn.cursor()
    
    chat_id_list = [chat_id]
    list_user = [chat_id, name, last_name, status, days,time, mode]
...
#Получаем id пользователя for function create_dict_list_allinfo
def create_id_list():
    check_init()
    conn=sqlite3.connect(file_name)
    curs = conn.cursor()
...
def create_dict_list_allinfo():
    ...
    for el in id_list:
        ...
        list_now_id.append(curs.fetchone()[0])
        curs.execute("""SELECT time FROM users_db WHERE id = ?""", (num,))
        list_now_id.append(curs.fetchone()[0])
        curs.execute("""SELECT mode FROM users_db WHERE id = ?""", (num,))
        list_now_id.append(curs.fetchone()[0])
